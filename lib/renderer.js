...
class Renderer {
  ...
  constructor(compiler, config) {
    ...
    this.useStore = (config && config.useStore) || storeEnum.AUTO;
  ...
  getVueClass() {
    ...
    this.vueClass = Vue;
    return Promise.resolve(this.vueClass);
  }
  /**
  ...
  getVueInstance(path, context) {
    ...
    return this.getSourceCodeString(path).then(code => {
      const sandbox = vm.createContext(Object.assign(global, {module, require}));
      const vueConfig = vm.runInContext(code, sandbox);
      const self = this;
      const isStore = this.useStore === storeEnum.AUTO ? (!!vueConfig.store) : (this.useStore === storeEnum.ON);
      ...
      return this.getVueClass().then(VueClass => Promise.resolve(new VueClass(vueConfig)));
    });
  }

  /**
   * get compiled source code string
   *
   * @param {string} path
   * @returns {Promise}
   *
